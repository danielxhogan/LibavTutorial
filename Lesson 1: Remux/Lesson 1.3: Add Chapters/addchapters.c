 ...
 * it also takes in a text file with chapter information in it and embeds
 * the information into the output file. It is roughly the equivalent of
...
int main(int argc, char **argv)
{
  ...
  AVFormatContext *v_fmt_ctx = NULL, *ch_fmt_ctx = NULL, *out_fmt_ctx = NULL;
  ...
  if (argc != 5) {
    printf("\nUsage: %s <video file> <chapter file> <output file> <title>\n\n\t"
    "This program performs all the same operations as cpmetadata.c. In addition,\n\t"
    "it will also take in a text file with chapter information in it and embed\n\t"
    "the information into the output file. It will be roughly the equivalent of\n\t"
    "the following ffmpeg command:\n\n\t"
    "ffmpeg -i \"<video file>\" -i \"<chapter file>\" -map_metadata 1 -c copy \\\n\t\t"
    "-metadata title=\"<title>\" \"<output file>\"\n"
    , argv[0]);
    return 0;
  }
  ...
  if ((ret = avformat_open_input(&ch_fmt_ctx, ch_filename, ch_fmt, NULL)) < 0) {
  ...
  if ((ret = initialize_stream(&v_stream_idx, out_fmt_ctx, v_fmt_ctx,
  ...
  if ((ret = initialize_stream(&a_stream_idx, out_fmt_ctx, v_fmt_ctx,
  ...
  avformat_close_input(&ch_fmt_ctx);
